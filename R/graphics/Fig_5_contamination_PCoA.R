...
plot_pcoa_eDNA_and_trawl <- function() {
  ...
  saveRDS(stat_plot,"output/PCoA_SSP_colors.RDS")
  ...
  #pdf("figures/Fig_S10_Conta_axes1_2.pdf", width = 17/2.54, height = 8/2.54) 
  jpeg("figures/Fig_S10_Conta_axes1_2.jpeg",width=17,height=8,res=300,units="cm")
  
  par(mar=c(2,2.1,1,0),mfrow=c(1,2))
  get_pcoa_plot(data=stat_plot[,c(1,2)], lbl = lbl,
                xlim=c(-0.5,0.5),ylim=c(-0.6,0.5),ax=c(1,2),Traits=stat_plot$type_region,
  ...
  text(0.02, 0.4, "eDNA, < 100 m", adj = 0, cex = 0.7)
  text(0.22, -0.2, "eDNA, > 100 m", adj = 0, cex = 0.7)
  text(-0.2, 0.03, "trawl, < 100 m", adj = 1, cex = 0.7)
  text(-0.23, -0.285, "trawl, > 100 m", adj = 0, cex = 0.7)
  ...
  points(x=stat_plot[which(stat_plot$type == "trawling"),]$longitude_edna-dx,
         ...
         bg=stat_plot[which(stat_plot$type == "trawling"),]$col,pch=22,
         col="black",cex=0.9,lwd=0.35)
  
  points(x=stat_plot[which(stat_plot$type == "eDNA"),]$longitude_edna+dx,
         y=stat_plot[which(stat_plot$type == "eDNA"),]$latitude_edna+dy,
         bg=stat_plot[which(stat_plot$type == "eDNA"),]$col,pch=21,col="black",
         cex=0.9,lwd=0.35)
  ...
  # Fig S6 ---------------------------------------------------------------------
  jpeg("figures/Fig_S10_Conta_axes2_3.jpeg", width = 17, height = 8,res = 300,units = "cm")

  #pdf(file= "figures/Fig_S10_Conta_axes2_3.pdf",width=14/2.54,height=6/2.54)
  par(mar=c(2,2,1,0.8),mfrow=c(1,2), mgp = c(3,1,0))
  get_pcoa_plot(data=stat_plot[,c(1,3)], lbl = lbl,
                xlim=c(-0.4,0.4),ylim=c(-0.5,0.5),ax=c(1,2),Traits=stat_plot$region,
  ...
  get_pcoa_plot(data=stat_plot[,c(2,3)], lbl = lbl,
                xlim=c(-0.5,0.5),ylim=c(-0.5,0.5),ax=c(1,2),Traits=stat_plot$region,
                xlab="",ylab="",col_bgf="grey76",colsclass=c("black", "black"),
                cex.txt = 0.27, offset = 0,
                col_vect=as.character(stat_plot$col),
                pos=NA,
...
###############################################################################@
jpeg("figures/Fig_S10_Conta_axes1_2_3_4.jpeg",width=17,height=16,res=300,units="cm")

par(mar=c(2,2.1,1,0),mfrow=c(2,2))
get_pcoa_plot(data=stat_plot[,c(1,2)], lbl = lbl,
              xlim=c(-0.5,0.5),ylim=c(-0.6,0.5),ax=c(1,2),Traits=stat_plot$type_region,
              xlab="",ylab="",col_bgf="grey76",colsclass=rep("black", 4),
              cex.txt=0.4, offset = 0,
...
text(-0.2, 0.03, "trawl, < 100 m", adj = 1, cex = 0.7)
text(-0.23, -0.285, "trawl, > 100 m", adj = 0, cex = 0.7)
mtext(text="e",side=3,adj = 0, cex = 1)
legend("bottomleft", legend = c("eDNA", "trawling"), pch = c(21,22), cex =0.7,box.lwd = 0.5 , bg = "white" )
# map
par(mar=c(2,1.6,1,0.3)) 
plot(0,0, type='n', xaxt='n', yaxt='n', xlab='', ylab='', bty='n')
mtext(text="f",side=3,adj = 0, cex = 1)
par(new = "T")
pre_map(GISdata, xlim = c(-5,0), ylim = c(43,48))
# plot points 
dx <- 0.03
...
title(xlab = xlab, line = 0.8, cex.lab = 0.7)            
title(ylab = ylab, line = 1.4, cex.lab = 0.7)
mtext(text="g",side=3,adj = 0, cex = 1)
legend("bottomleft", legend = c("eDNA", "trawling"), pch = c(21,22), cex =0.5,box.lwd = 0.5  )
...
mtext(text="h",side=3,adj = 0, cex = 1)
dev.off()
}
